...
OverrideWeights = 0 # override weights and TPmode of global tag with user input sqlite files, 0 means weights from global tag 
oneFile = 0 # Run over one file as a test (recommended before running over large datasets to test incompatibility issues)
WeightsWP = "kill+tag"  # odd weights working point. Options: [2p5Prime, 0p5Prime] 
...
config.JobType.pyCfgParams = [
                               ...
                               'OddWeightsGroupSqliteFile=NONE', # weights group for each strip - defines which set of ODD weights each strip should use 
                                ...
                                'TPModeTag=FROMDB', # TPMode, aka electronics configuration
