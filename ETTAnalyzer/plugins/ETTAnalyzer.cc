...
void ETTAnalyzer::analyze(const edm::Event &e, const edm::EventSetup &c)
{
  ...
  // Find out in which BX the first collision in train, isolated bunch, and last
  // collision in train have fired. In case of pre firing it will be in BX 1 or
  ...
  // we want to use only those events which comes as a result of the begining of
  // the bunch train, each bunch train can have crossings multiple times. if the
  // egamma triggering happended one bx before the begining of bunch train i.e.
  // egamma trigger pre-fired then we need to correct for this effect. The data
  // is written only once, for all the five bunch crossings. if the egamma
  // trigger was prefired then (-1 bx) then everything needs to be translated.
       ...
       ++itBX)
  {
    ...
    for (l1t::EGammaBxCollection::const_iterator egamma =
             EGammaBxCollection->begin(itBX);
         egamma != EGammaBxCollection->end(itBX); ++egamma)
    {
      ...
      for (size_t i = 0; i < egammaPtCuts_.size(); ++i)
      {
        ...
        if (egamma->pt() >= egammaPtCuts_.at(i))
        {
          ...
          // denominator_egamma_firstbunch.at(i)->Fill(egamma->eta(),
          // egamma->phi());
          // egamma_eta_phi_firstbunch.at(i).at(index)->Fill(egamma->eta(),
          // egamma->phi());
          //}
          ...
          if ((bool)egamma->hwIso())
          {
            //  std::cout<<" isolated bx, eta, counter "<<itBX - bxShiftFirst<<" "<< egamma->hwEta()<<"  "<<isocounterp2<<std::endl;
             ...
             if(savePreFireInfo_)
            first_ibx_vs_ieta_Iso->Fill(itBX - bxShiftFirst, egamma->hwEta() );
          ...
          // ----------------------------------------------------------
           first_ibx_vs_ieta_NonIso->Fill(itBX - bxShiftFirst, egamma->hwEta());
      ...
      if (egamma->pt() >= egammaPtCuts_.at(i)) {
           ...
           if ((bool)egamma->hwIso()) {
            //  std::cout<<" isolated "<<correctedBx<<"  "<< egamma->hwEta()<<std::endl;
             last_ibx_vs_ieta_Iso->Fill(correctedBx, egamma->hwEta() );
          ...
          last_ibx_vs_ieta_NonIso->Fill(correctedBx, egamma->hwEta() );
         }
       }
     }
   }
  ...
  double rawEventTime =
      (double)(timeStampHigh) + ((double)(timeStampLow) / 1000000.);
  double eventTime =
      rawEventTime - startTime_; // Notice the subtraction of the "starttime"
  ...
  //    std::cout<<"FATAL EXCEPTION: "<<"Following Not Found: EEdigistoken_
  //    "<<std::endl; exit(0);
  //  }
  ...
  if (not e.getByToken(EBdigistoken_, EBdigis))
  {
    std::cout << "FATAL EXCEPTION: "
              << "Following Not Found: EBdigistoken_ " << std::endl;
    exit(0);
  }
  ...
  //  std::cout << "Right before EB digis loop" << std::endl;
   int numDigis = 0;
   for ( EBDigiCollection::const_iterator hitItr = EBdigis->begin(); hitItr !=
  EBdigis->end(); ++hitItr ) {

    EBDataFrame df(*hitItr); // full readout runs crashing here. Does not crash
  on same EBdigis instance every time. Memory issue? const EBDetId & id =
  df.id(); const EcalTrigTowerDetId towid = (*eTTmap_).towerOf(id);
  ...
  theMapping_->getTriggerElectronicsId(df.id());
    ...
    for(int i=0; i<10;++i){
      //std::cout<<" tower (eta, phi): ("<<towid.ieta() << ",
  "<<towid.iphi()<<")"
      //     <<" xtal (ix, iy): ("<<id.ix() <<", "<<id.iy()<<")"
      //     <<" ADC for EEDataFrame: "<<j << "  sample number "<<i<<"
  "<<df.sample(i).adc()<<std::endl;

    //  // these, tower_eta for example, is set to size 4092, but elements being
  filled for each digi per rec hit tower_eta[countNadc] = towid.ieta();
  ...
  //  // EE only
  //  for ( EEDigiCollection::const_iterator hitItr = EEdigis->begin(); hitItr
  //  != EEdigis->end(); ++hitItr ) {
  ...
  {
    for (EcalRecHitCollection::const_iterator rechitItr = rechitsEB->begin();
         rechitItr != rechitsEB->end(); ++rechitItr)
    {
  ...
  //    // For each EB Rec Hit
  //    for ( EcalRecHitCollection::const_iterator rechitItr =
  //    rechitsEB->begin(); rechitItr != rechitsEB->end(); ++rechitItr ) {
...
void ETTAnalyzer::beginRun(const edm::Run &r, const edm::EventSetup &c)
{
  ...
  if (!gtUtil_->getAlgBitFromName(algoNameLastBxInTrain_,
                                  algoBitLastBxInTrain_))
  {
    edm::LogWarning("L1TObjectsTiming")
        << "Algo \"" << algoNameLastBxInTrain_
        << "\" not found in the trigger menu " << gtUtil_->gtTriggerMenuName()
        << ". Could not retrieve algo bit number.";
  }
  ...
  {
    edm::LogWarning("L1TObjectsTiming")
        << "Algo \"" << algoNameIsoBx_ << "\" not found in the trigger menu "
...
// ------------
void ETTAnalyzer::endJob() { ETTAnalyzerTree->Write(); }

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void ETTAnalyzer::fillDescriptions(
    edm::ConfigurationDescriptions &descriptions)
{
  ...
  desc.addUntracked<std::string>("useAlgoDecision", "initial")
      ->setComment("Which algo decision should be checked [initial, "
                   "intermediate, final].");
  desc.addUntracked<std::vector<double>>("egammaPtCuts", {20., 10., 30.})
      ->setComment("List if min egamman pT vaules");
